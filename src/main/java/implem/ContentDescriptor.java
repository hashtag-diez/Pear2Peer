...
public class ContentDescriptor extends ContentTemplate implements ContentDescriptorI, Serializable {
    ...
    protected boolean matchInterpreters(ContentTemplateI request) {
        if (request.getInterpreters().size() == 0 || _interpreters.size() == 0)
            return false;
    ...
    protected boolean matchComposers(ContentTemplateI request) {
        if (request.getComposers().size() == 0 || _composers.size() == 0)
            return false;
